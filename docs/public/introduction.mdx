...
### Codex CLI experimental path

Codex CLI installation is available through the npx installer, but it is currently documented as **experimental/internal RC**, not as public-stable Claude Code parity:

```bash
npx claude-mem install --ide codex-cli
```

This path writes Codex-specific hook/transcript configuration while keeping the plugin payload in the regular Claude-Mem marketplace install tree. See [Codex CLI Integration](codex-cli) before using it.
...
<CardGroup cols={2}>
  ...
  <Card title="Codex CLI" icon="terminal" href="/codex-cli">
    Experimental/internal RC install path
  </Card>
